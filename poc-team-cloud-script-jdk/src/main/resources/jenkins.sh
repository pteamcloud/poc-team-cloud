...
sudo sed -i "s/JENKINS_USER=jenkins/JENKINS_USER=$USER/g"           /etc/sysconfig/jenkins
sudo sed -i 's/HTTP_PORT=8080/HTTP_PORT=18080/g'                       /etc/sysconfig/jenkins
mkdir /home/$USER/jenkins
sudo sed -i "s/\/var\/lib\/jenkins/\/home\/$USER\/jenkins/g"       /etc/sysconfig/jenkins
sudo sed -i "s/JENKINS_ARGS=/#JENKINS_ARGS=/g"           /etc/sysconfig/jenkins
sudo echo 'JENKINS_ARGS="--httpPort=$HTTP_PORT --ajp13Port=$AJP_PORT"' | sudo tee -a /etc/sysconfig/jenkins > /dev/null
...
# Jenkins runs on port 18080 by default. Let's make sure port 18080 is open:
if [ "$isCentOs7" == true ]
then
    sudo firewall-cmd --zone=public --add-port=18080/tcp --permanent
    sudo firewall-cmd --reload
else
    sudo iptables -A INPUT -p tcp -m tcp --dport 18080 -j ACCEPT
...
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin ant
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin credentials
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin deploy
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin git-client
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin git
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin github-api
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin github-oauth
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin github
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin gcal
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin google-oauth-plugin
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin greenballs
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin javadoc
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin ldap
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin mailer
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin mapdb-api
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin maven-plugin
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin external-monitor-job
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin oauth-credentials
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin pam-auth
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin scm-api
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin ssh-agent
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin ssh-credentials
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin ssh-slaves
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin subversion
sudo java -jar $CWD/jenkins-cli.jar -s http://localhost:18080/ install-plugin translation
...
# Let's start Jenkins
sudo java -jar jenkins-cli.jar -s http://localhost:18080 safe-restart
